...
KoColorTransformation* KisHSVAdjustmentFilter::createTransformation(const KoColorSpace* cs, const KisFilterConfiguration* config) const
{
    ...
    if (config) {
        ...
        if (cs->profile()->hasColorants()){
            QVector <double> colorants = cs->profile()->getColorantsxyY();
            params["lumaRed"]   = colorants[2];
            params["lumaGreen"] = colorants[5];
            params["lumaBlue"]  = colorants[8];
        } else {
            params["lumaRed"]   = 0.2126;
            params["lumaGreen"] = 0.7152;
            params["lumaBlue"]  = 0.0722;
        }
    }
